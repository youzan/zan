...
#ifndef _SW_MODULE_H_
#define _SW_MODULE_H_
...
static sw_inline void swArgs_push_null(void)
{
    ...
    if (buffer->size < sizeof(swVal) && swString_extend(buffer, buffer->size * 2) < 0)
    {
        return ;
    }
...
{
    swString *buffer = SwooleG.module_stack;
    if (buffer->size < sizeof(swVal) + sizeof(long) && swString_extend(buffer, buffer->size * 2) < 0)
    {
        return ;
    }
...
{
    swString *buffer = SwooleG.module_stack;
    if (buffer->size < sizeof(swVal) + sizeof(uint8_t) && swString_extend(buffer, buffer->size * 2) < 0)
    {
        return ;
    }
    swVal *val = (swVal *) (buffer->str + buffer->length);
...
{
    swString *buffer = SwooleG.module_stack;
    if (buffer->size < sizeof(swVal) + sizeof(double) && swString_extend(buffer, buffer->size * 2))
    {
        return ;
    }
...
static sw_inline void swArgs_push_string(const char *str, int length)
{
    ...
    if (buffer->size < size && swString_extend(buffer, size) < 0)
    {
        return ;
    }
...
{
    swString *buffer = SwooleG.module_stack;
    if (buffer->size < sizeof(swVal) + sizeof(long) && swString_extend(buffer, buffer->size * 2))
    {
        return NULL;
    }
...
{
    swString *buffer = SwooleG.module_stack;
    if (buffer->size < sizeof(swVal) + sizeof(uint8_t) && swString_extend(buffer, buffer->size * 2))
    {
        return NULL;
    }
...
{
    swString *buffer = SwooleG.module_stack;
    if (buffer->size < sizeof(swVal) + sizeof(double) && swString_extend(buffer, buffer->size * 2) < 0)
    {
        return NULL;
    }
...
{
    swString *buffer = SwooleG.module_stack;
    if (buffer->size < (sizeof(swVal) + len + 1) && swString_extend(buffer, sizeof(swVal) + len + 1))
    {
        return NULL;
    }
