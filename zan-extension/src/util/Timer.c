...
int swTimer_init(swTimer* timer,long msec)
{
    ...
    int iRet = (swIsTaskWorker())? swSystemTimer_init(msec, SwooleG.use_timer_pipe):
    ...
    if (timer_cfg.use_time_wheel && (swIsWorker() || swIsOthers()))
    {
...
static int swSystemTimer_init(long interval, int use_pipe)
{
    ...
    if (SwooleG.use_timerfd && swSystemTimer_timerfd_set(timer, interval) >= 0)
    {
        timer->use_pipe = 0;
    }
    else if (!SwooleG.use_timerfd)
    {
...
static swTime_wheel* swTime_wheel_create(long precision,long max_timeout)
{
	...
	{
		swWarn("max timeout and time precision set error.");
		return NULL;
	}
