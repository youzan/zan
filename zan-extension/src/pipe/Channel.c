...
static void swChannel_debug(swChannel *chan);
...
swChannel* swChannel_create(int size, int maxlen, int flag)
{
    ...
    //use lock
    if ((flag & SW_CHAN_LOCK) && swMutex_create(&object->lock, 1) < 0)
    {
         swWarn("swChannel_create: mutex init fail");
         return NULL;
    }
    //use notify
    if ((flag & SW_CHAN_NOTIFY) && swPipeNotify_auto(&object->notify_fd, 1, 1) < 0)
    {
...
int swChannel_push_withLock(swChannel *object, void *in, int data_length)
{
    ...
    int ret = swChannel_push(object, in, data_length);
...
int swChannel_pop_withLock(swChannel *object, void *out, int buffer_length)
{
    assert(object->flag & SW_CHAN_LOCK);
    object->lock.lock(&object->lock);
    int n = swChannel_pop(object, out, buffer_length);
    object->lock.unlock(&object->lock);
    return n;
}
